...
#target = cv.imread('target.tif')
#source = cv.imread('source.tif')

target = cv.imread('/media/sf_Shared_Folder/200X/SOB_B_A-14-22549G-200-029.png')
target = cv.cvtColor(target, cv.COLOR_BGR2RGB)
for fn in glob('/media/sf_Shared_Folder/200X/*'):
	source = cv.imread(fn)
	...
	transformedbgr = cv.cvtColor(transformed, cv.COLOR_RGB2BGR)
	out_fn = os.path.basename(os.path.splitext(fn)[0])
	cv.imwrite('output/'+out_fn+'.png',transformedbgr,[int(cv.IMWRITE_PNG_COMPRESSION),0])
